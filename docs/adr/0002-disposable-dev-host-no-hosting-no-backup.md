...
and it is **not backed up**: no DO automatic backups, no snapshots. Everything that
matters is pushed to a forge; the host is recreated by `tools/provision-droplet.sh`.
...
- **Agents effectively have root.** `socle`'s dev stack needs Docker, and the `docker`
  ...
  holds nothing unique — it would not be otherwise. It is also why `~/.ssh` is never
