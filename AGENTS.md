...
<!--
Keep this file short. Put file-specific details in .claude/rules/ and keep
command-triggered constraints here so agents see them before running commands.
-->
...
- Put shared repository instructions in this file. `CLAUDE.md` imports it; add
  Claude-specific instructions there only when needed. Global preferences live
  in `agents/AGENTS.md`, linked from the tools' user configuration directories.
- Before editing files, read the applicable documents in `.claude/rules/`, using
  their `paths:` frontmatter to select them. Claude loads these automatically;
  other agents must read them explicitly.
