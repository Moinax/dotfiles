 ...
 * point into the project picker.
 *
 * There was briefly a second command (Dev Worktree, on Mod+Ctrl+Return) that
 * rendered this exact list with the two actions swapped, so that Enter landed
 * on the worktrees instead of on the main checkout. That is a thing an
 * ActionPanel already does, so the second command bought a keybind and a
 * manifest entry to move one action up one row. Dropped.
 *
