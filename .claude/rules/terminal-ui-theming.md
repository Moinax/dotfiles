...
The law itself is in `AGENTS.md` — an explicit bg on a translucent pane paints an
...
- `ccstatusline/settings.json.tmpl` — `overrideBackgroundColor` stays unset: with
  `flexMode: "full"`, which is set and stays, it plated the whole statusline
