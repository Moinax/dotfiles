...
#
# The host is deliberately disposable — see docs/adr/0002. Nothing unique lives
# on it, so this script, not a snapshot, is what brings it back. Every phase is
# idempotent: re-running it on a live host is the repair path, not a reinstall.
#
#   provision-droplet.sh create     create the droplet (cloud-init: user + key)
#   provision-droplet.sh setup      copy this script over and run its remote half
#   provision-droplet.sh firewall   deny-all inbound; run only once Tailscale is up
#   provision-droplet.sh status     what exists, what is reachable, what is missing
#   provision-droplet.sh target     user@address to reach the host (public, else tailnet)
#   provision-droplet.sh pair       mint a pairing code for one more device
#   provision-droplet.sh destroy    delete the droplet and its firewall
#   provision-droplet.sh remote     the half that runs ON the host (called by setup)
#
...
cmd_setup() {
    local target
    target=$(host_target) \
        || { err "Cannot reach $DROPLET_NAME on its public IP or the tailnet"; exit 1; }
...
cmd_firewall() {
    ...
    if ! ssh -o BatchMode=yes -o ConnectTimeout=10 "$(host_target)" \
             'tailscale status --json >/dev/null 2>&1 && tailscale ip -4' 2>/dev/null; then
        err "Tailscale is not up on the host — the firewall would lock you out"
        info "Run the wizard's Tailscale step first, then come back to this"
        return 1
    fi
