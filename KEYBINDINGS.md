...
| T3 Code | `Mod+Ctrl+Return` | Vicinae project picker with thread, running and blocked counts. **Enter** on a project lists its threads; **New thread** opens a draft in T3 Code, where you write the first prompt. An unknown project shows the same picker with no existing threads. T3 Code starts and adds the project if needed. Drafts use the app's model and workspace defaults. |
